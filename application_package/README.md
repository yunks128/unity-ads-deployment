...
**TODO:** There is a whole bunch of manually set AWS parameters as required by the Dockstore deployment, which could be replaced by Terraform variables (search for `/DeploymentConfig/dev/` in `AWS Console: AWS Systems Manager -> Parameter Store`).
...
* Webhook URL:
  ...
  - URL is accessible through `AWS Console: API Gateway -> Unity API Gateway -> Stages` - select the stage and URL appears under `Invoke URL` in the top level toolbar. For example, the Webhook URL follows the convention `https://value1.execute-api.us-west-2.amazonaws.com/dev/ads/dockstore/webhooks/github/enqueue` for the development deployment.
* Webhook secret:
